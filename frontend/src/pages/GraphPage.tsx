...
export default function GraphPage() {
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({
      ...
      style: [
        {selector:'node[kind="paper"]',style:{label:"data(label)","background-color":"#3f6c5d",color:"#26322d","font-size":"10px","text-wrap":"ellipsis","text-max-width":"130px","text-valign":"bottom","text-margin-y":8}},
        {selector:'node[kind="keyword"]',style:{label:"data(label)","background-color":"data(color)",shape:"round-rectangle",width:22,height:22,"font-size":"9px","text-valign":"bottom","text-margin-y":6}},
        {selector:"edge",style:{width:1.5,"line-color":"#a5b4ad","target-arrow-color":"#a5b4ad","target-arrow-shape":mode==="citation"?"triangle":"none","curve-style":"bezier"}},
  ...
  return <main className="page">
    ...
    <div className="graph-layout"><div ref={container} className="graph-canvas" aria-label="ナレッジグラフ"/><aside className="panel graph-info" style={{position:"static",height:"auto",background:"white",color:"inherit",padding:"1rem"}}>{selected ? <><p className="eyebrow">{String(selected.kind)}</p><h2>{String(selected.label)}</h2>{selected.kind==="paper"&&<><p>{String(selected.status)}・{String(selected.year||"発行年未登録")}</p><button onClick={()=>navigate(`/papers/${String(selected.id)}`)}>論文詳細を開く</button></>}</>:<p className="muted">ノードを選ぶと概要を表示します。マウスホイールで拡大・縮小、ドラッグで移動できます。</p>}</aside></div>
  </main>;
}
