...
export default function DetailPage({readonly}: {readonly: boolean}) {
  ...
  return <main className="page">
    ...
    {tab === "メモ" && <section className="panel"><div className="page-header"><div><strong>定型Markdownメモ</strong><p className="muted">Markdownの色分け、箇条書き継続、Tabインデント、検索、元に戻す／やり直しを利用できます。</p></div><span className={dirty ? "unsaved" : "muted"}>{dirty ? "未保存" : "保存済み"}</span></div><div className="note-view-switch" aria-label="メモ表示"><button type="button" className={noteView === "edit" ? "active" : ""} aria-pressed={noteView === "edit"} onClick={() => setNoteView("edit")}>編集</button><button type="button" className={noteView === "preview" ? "active" : ""} aria-pressed={noteView === "preview"} onClick={() => setNoteView("preview")}>プレビュー</button></div>{noteView === "edit" ? <MarkdownEditor value={note} onChange={setNote} onSave={() => {if (!readonly && dirty) void saveNote();}} readonly={readonly} /> : <MarkdownPreview content={note} />}<div className="actions"><button onClick={saveNote} disabled={readonly || !dirty}>保存（Ctrl+S）</button></div></section>}
